...
export function CandidacyForm(){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){
  ...
  try{
   ...
   if(!applicationId){const{data:createdId,error}=await supabase.rpc("submit_candidate_application",{p_first_name:String(data.get("first_name")||""),p_middle_name:String(data.get("middle_name")||""),p_last_name:String(data.get("last_name")||""),p_phone:String(data.get("phone")||""),p_email:String(data.get("email")||""),p_parent_seniority:Number(data.get("parent_seniority")),p_relationship:String(data.get("relationship")||""),p_position_name:positionMap[String(data.get("position")||"")]||"",p_biography:String(data.get("biography")||""),p_manifesto:String(data.get("manifesto")||""),p_children:children});if(error||!createdId)throw error??new Error("Missing application identifier");applicationId=createdId;setPendingApplicationId(createdId)}
   ...
   setStatus("success");setPendingApplicationId(null);form.reset();setChildrenCount(1);
  }catch(error){console.error("Candidacy submission failed",error);setStatus(applicationId?"photo-error":"error")}
 }
 return <form className="card candidate-form" onSubmit={submit}>
  ...
  <button className="btn btn-primary" type="submit" disabled={status==="sending"}><Send size={18}/>{status==="sending"?"Submitting application...":status==="photo-error"?"Retry photo upload":"Submit my candidacy"}</button>
  {status&&<p className={status==="error"?"status-message error":"status-message"}>{status==="sending"?"Submitting application...":status==="success"?"Your candidacy has been registered and is awaiting KCS review.":status==="photo-error"?"Your form is registered, but the photo upload failed. Keep this page open and retry the photo upload.":"The application could not be submitted. Check your information or try again."}</p>}
