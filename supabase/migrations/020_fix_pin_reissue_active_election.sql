-- Always reissue the PIN for the voter record belonging to the election
-- whose login window is currently open, even if an older row was selected.
begin;
...
begin
 ...
 values(v.election_id,auth.uid(),'VOTER_PIN_REISSUED','voter',v.id::text,jsonb_build_object('voter_code',v.voter_code,'active_election_forced',true),previous,encode(digest(payload,'sha256'),'hex'));
